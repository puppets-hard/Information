...
$(function(){
        ...
        $.ajax({
            ...
            success: function(resp){
                if (resp.errno == "0"){
                    ...
                    comment_html += '<div class="comment_text fl">' + comment.content + '</div>'
                    ...
                    comment_html += '<form class="reply_form fl" data-commentid="' + comment.news_id + '" data-newsid="' + news_id + '">'
    ...
    $('.comment_list_con').delegate('a,input','click',function(){  // delegate() 方法为指定的元素（属于被选元素的子元素）添加一个或多个事件处理程序，并规定当这些事件发生时运行的函数

        var sHandler = $(this).prop('class');  // prop() 方法设置或返回被选元素的属性和值。
        // alert(sHandler) sHandler是一个字符串

        if(sHandler.indexOf('comment_reply')>=0)  // indexOf() 方法可返回某个指定的字符串值在字符串中首次出现的位置
        {
            $(this).next().toggle();  // toggle() 方法在被选元素上进行 hide() 和 show() 之间的切换
        }
        ...
        if(sHandler.indexOf('reply_sub')>=0)
        {
            ...
            var comment = $this.prev().val()  // prev() 方法返回被选元素的前一个同级元素
