$(function(){
	...
	$('.register_btn').click(function(){
		...
		generateImageCode()  // 打开注册框的时候请求验证码图片
	})
	...
	$('.to_register').click(function(){
		$('.login_form_con').hide();
		$('.register_form_con').show();
        generateImageCode()  // 切换注册框的时候请求验证码图片
	})
...
function sendSMSCode() {
    ...
    $.ajax({
        ...
        // 成功
        success: function (resp) {
            if (resp.errno == "0"){  // 如果成功
                var num = 60;  // 倒计时60秒
                // 设置计时器
                t = setInterval(function () {
                    if (num == 1){  // 倒计时结束
                        clearInterval(t);  // 清除计时器
                        $(".get_code").html("获取验证码");  // 将信息改回原始文本
                        $(".get_code").attr("onclick", "sendSMSCode();");  // 回复点击事件
                    }
                    else{
                        num -= 1;
                        $(".get_code").html(num+"秒")
